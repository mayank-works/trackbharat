// frontend/src/components/common/GradientWaves.tsx
...
const GradientWaves: React.FC<GradientWavesProps> = ({
  ...
  useEffect(() => {
    ...
    (container as any).__gradientWaves = { renderer, program, mesh };
    ...
    return () => {
      ...
      delete (container as any).__gradientWaves;
      try {
  ...
  useEffect(() => {
    const container = containerRef.current;
    if (!container) return;
    const ctx = (container as any).__gradientWaves;
    if (!ctx) return;
    const { program } = ctx;
